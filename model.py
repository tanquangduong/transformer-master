...
class LayerNormalization(nn.Module):
    def __init__(self, eps: float = 10**-6) -> None:
        ...
        self.bias = nn.Parameter(torch.zeros(0))  # Additive parameter
...
class FeedForwardBlock(nn.Module):
    def __init__(self, d_model: int, d_ff: int, dropout: float) -> None:
        ...
        self.linear_1 = nn.Linear(
            d_model, d_ff
        )  # W1 and B1, because bias=True by default
        self.dropout = nn.Dropout(dropout)
        self.linear_2 = nn.Linear(
            d_ff, d_model
        )  # W2 and B2, because bias=True by default
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        query = query.view(query.shape[0], query.shap[1], self.h, self.d_k).transpose(
            1, 2
        )
